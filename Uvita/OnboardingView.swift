...
struct SkinTypePage: View {
    ...
    // All 6 Fitzpatrick types for display
    // grouped into the two model tiers
    let types: [(roman: String,
                 label: String,
                 desc: String,
                 example: String,
                 emoji: String,
        ...
        ("I",   "Very fair",
         ...
         "Pale white skin, often freckles",
         "👱🏻", .typeI_II),
        ("II",  "Fair",
         "Usually burns, tans minimally",
         "White skin, light hair",
         "🧑🏼", .typeI_II),
        ("III", "Medium",
...
struct AgePage: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            // Age factor info card
            VStack(spacing: 8) {
                HStack {
                    VStack(alignment: .leading,
                           spacing: 4) {
                        Text("Age factor (f_age)")
                            .font(.caption)
                            .foregroundColor(
                                .white.opacity(0.6))
...
struct ClothingPage: View {
    ...
    // Body diagram as simple text illustration
    var bodyDiagram: String {
        switch selected {
        case .swimwear:       return "🧍 Full exposure"
        case .tankShorts:     return "🧍 Arms + legs"
        case .tshirtShorts:   return "🧍 Lower legs"
        case .tshirtPants:    return "🧍 Arms only"
        case .longSleevesPants: return "🧍 Hands + face"
        case .fullyCovered:   return "🧍 Face only"
        case nil:             return "🧍"
        }
    }

    let options: [(opt: ClothingOption,
                   emoji: String,
                   desc: String)] = [
        (.fullyCovered,
         "🧥", "Full coverage — coat, hoodie, long everything"),
        (.longSleevesPants,
         "👔", "Long sleeves + pants — office or cool day"),
        (.tshirtPants,
         "👕", "T-shirt + pants — typical casual"),
        (.tshirtShorts,
         "🩳", "T-shirt + shorts — warm day"),
        (.tankShorts,
         "💪", "Tank top + shorts — hot day, beach"),
        (.swimwear,
         "🩱", "Swimwear — pool, beach, sunbathing"),
    ]
        ...
        VStack(spacing: 0) {
            ...
            VStack(spacing: 12) {
                ...
                Button {
                    ...
                    .padding(16)
                    .background(knowsLevel
                        ? Color.white
                        : Color.white.opacity(0.12))
                    .cornerRadius(14)
                }

                // Blood test input — shows if selected
                if knowsLevel {
                    HStack {
                        TextField("e.g. 45",
                                  text: $levelText)
                    ...
                    if let val = Double(levelText) {
                        Text(val < 30
                            ...
                            .font(.caption)
                            .foregroundColor(
                                val < 30 ? Color(red: 1, green: 0.4, blue: 0.4)
                                : val < 50 ? Color(red: 1, green: 0.7, blue: 0.2)
